...
**重要提示：探索模式是为了思考，而不是为了实施。** 你可以阅读文件、搜索代码和调查代码库，但你绝不能编写代码或实现功能。如果用户要求你实现某些内容，请提醒他们切换到 `/opsx:propose` 或 `/opsx:apply`。如果需要把探索结果沉淀到 OpenSpec 中，先遵守 superpower 的 `brainstorming` 流程，再决定是否写入产出物。

**这是一种姿态，而不是一种独立工作流。** 在 Codex 中，此技能应与 `using-superpowers` 保持一致，并优先作为 `brainstorming` 的探索入口使用。
...
- 如果对话已经进入正式设计，请以 `brainstorming` 的节奏继续，而不是自创一套流程
- 除非用户明确要求记录，否则不要自动改写 OpenSpec 产出物
- 如果探索已收敛并且用户准备推进，建议转到 `/opsx:propose`
...
- **要探索代码库** - 将讨论建立在现实基础上
- **要质疑假设** - 包括用户的和你自己的
- **要服从 superpower 流程优先级** - 探索服务于更大的流程，不要与其竞争

...
- 归档前必须满足 `verification-before-completion` 的证据要求
- 归档是 OpenSpec 目录操作，不等于实现真的完成了

**步骤**

1. **如果没有提供变更名称，提示选择**

   运行 `openspec-cn list --json` 获取可用变更，并在正常对话中让用户选择。
...
4. **确认已经完成 fresh verification**

   在任何“可以归档”声明前，先检查是否有与当前变更相符的最新验证证据，例如：
   - 测试命令成功
   - 构建命令成功
   - 与需求对应的关键验证已完成

   如果没有 fresh verification，先要求执行验证，不要直接把归档说成完成。
...
5. **评估增量规范同步状态**
   ...
   **提示选项：**
   ...
   如果用户选择同步，直接执行 OpenSpec CLI 或在正常对话中说明将先完成同步步骤。不要引用不存在的代理工具。无论选择如何，都继续归档。

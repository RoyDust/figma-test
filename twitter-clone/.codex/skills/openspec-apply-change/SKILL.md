...
**输入**：可选指定变更名称。如果省略，检查是否可以从对话上下文中推断。如果模糊或不明确，用正常对话让用户从可用变更中选择。

## 核心原则

- 先遵守 `using-superpowers`
- OpenSpec 提供任务与上下文，superpower 提供实现纪律
- 不要把此技能当成绕过 `test-driven-development`、`systematic-debugging`、`verification-before-completion` 的捷径
...
1. **选择变更**
   ...
   - 如果只存在一个活动变更，自动选择
   - 如果不明确，运行 `openspec-cn list --json` 获取可用变更，并在正常对话中让用户选择
...
6. **按 superpower 流程实现任务（循环直到完成或受阻）**

   对于每个待处理任务：
   - 显示正在处理哪个任务
   - 如果是功能或缺陷修复，先遵守 `test-driven-development`
   - 如果遇到错误、失败测试或未知行为，先遵守 `systematic-debugging`
   - 进行所需的代码更改
   - 保持更改最小化且专注
   - 在任务文件中标记任务完成：`- [ ]` → `- [x]`
   - 继续下一个任务
...
7. **完成或暂停时，显示状态**
   ...
   - 本次会话完成的任务
   - 总体进度："N/M 任务已完成"
   - 如果全部完成：先遵守 `verification-before-completion`，再建议归档
   - 如果暂停：解释原因并等待指导
...
- 在任何“完成”“修复”“通过”的表述前，先运行 fresh verification
...
- **仍然必须服从 superpower**：流程可以灵活，纪律不能跳过
